...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="Server">
  <style>
    ...
    .data-grid .grid-pager a, .data-grid .grid-pager span { 
      margin: 0 4px; 
      padding: 8px 12px; 
      border: 1px solid #e6f7ff; 
      border-radius: 6px; 
    ...
    /* 列宽设置 - 描述列最大化 */
    .col-serial { width: 60px; text-align: center; }
    ...
    .status-offline { background:#f0f0f0; color:#595959; border:1px solid #d9d9d9; }
    ...
    @media (max-width: 768px) {
      ...
      .col-serial, .col-id { display: none; }
    }
  ...
  <div class="page-wrapper">
    <div class="query-card" role="form" aria-labelledby="queryTitle">
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          <asp:Label runat="server" CssClass="form-label" AssociatedControlID="ddlLocation" Text="地点" />
    ...
    <div class="table-container">
      <asp:GridView ID="gvIncidents" runat="server"
        AutoGenerateColumns="False" CssClass="data-grid" GridLines="None" DataKeyNames="Incident_id"
        ...
        EmptyDataText="<tr><td colspan='8' class='empty-data'>暂无符合条件的事件数据</td></tr>">
        <Columns>
          ...
          <asp:BoundField DataField="Incident_id" HeaderText="ID" HeaderStyle-CssClass="col-id" ItemStyle-CssClass="col-id" />
          
          <asp:BoundField DataField="Incident_type" HeaderText="类型" HeaderStyle-CssClass="col-type" ItemStyle-CssClass="col-type" />
          
          <asp:BoundField DataField="Occur_time" HeaderText="发生时间" DataFormatString="{0:yyyy-MM-dd}" HeaderStyle-CssClass="col-time" ItemStyle-CssClass="col-time" />
          
          <asp:BoundField DataField="Location" HeaderText="地点" HeaderStyle-CssClass="col-loc" ItemStyle-CssClass="col-loc" />
          
          <%-- 简化：状态列可选保留。此处保留显示“已公开” --%>
          ...
          <asp:TemplateField HeaderText="事件描述" HeaderStyle-CssClass="col-desc" ItemStyle-CssClass="col-desc">
            <ItemTemplate>
              ...
              <asp:LinkButton ID="btnViewDetail" runat="server" CssClass="toggle-desc" CommandName="ViewDetail" CommandArgument='<%# Eval("Incident_id") %>' Text="查看全文" />
            </ItemTemplate>
          </asp:TemplateField>

          <%-- 小白讲解：新增“操作”列。只有管理员、且当前行状态为“已公开”时显示“下架”按钮 --%>
          <%-- 简化：去掉操作列 --%>
        </Columns>
      </asp:GridView>
    </div>
    ...
    <%-- 详情弹窗（同页显示全文） --%>
    <asp:Panel ID="pnlDetail" runat="server" Visible="false" Style="position:fixed;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.35);z-index:9999;display:flex;align-items:center;justify-content:center;">
      <div style="width:80%;max-width:1000px;background:#fff;border-radius:16px;box-shadow:0 16px 40px rgba(0,0,0,0.2);overflow:hidden;">
        <div style="padding:18px 24px;border-bottom:1px solid #f0f0f0;background:linear-gradient(135deg,#f8fbff,#e6f7ff);color:#0c5aa6;font-weight:800;">
        ...
        <div style="padding:18px 24px;border-top:1px solid #f0f0f0;text-align:right;">
          <asp:Button ID="btnCloseDetail" runat="server" CssClass="btn-primary" Text="关闭" OnClick="btnCloseDetail_Click" />
        </div>
      </div>
    </asp:Panel>
  </div>
