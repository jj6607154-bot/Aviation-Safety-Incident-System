...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
  <style>
    ...
    .toggle-desc { display:inline-block; margin-top:8px; font-size:12px; color:#1890ff; text-decoration:none; }
  ...
  <div class="page-wrapper">
    <div class="card">
      ...
      <asp:GridView ID="gvPending" runat="server" CssClass="audit-grid" AutoGenerateColumns="false" DataKeyNames="Incident_id" OnRowCommand="gvPending_RowCommand" AllowPaging="true" PageSize="5" GridLines="None"
      ...
      <asp:TemplateField HeaderText="事件描述" HeaderStyle-CssClass="col-desc" ItemStyle-CssClass="col-desc">
        <ItemTemplate>
          ...
          <a href="#" class="toggle-desc">展开</a>
        </ItemTemplate>
      </asp:TemplateField>
      
      <asp:TemplateField HeaderText="状态">
        <ItemTemplate>
          <span class='<%# StatusClass(Eval("Incident_status")) %>'><%# Eval("Incident_status") %></span>
        </ItemTemplate>
      </asp:TemplateField>
      ...
      <asp:TemplateField HeaderText="审核操作" HeaderStyle-CssClass="col-ops" ItemStyle-CssClass="col-ops">
        <ItemTemplate>
          ...
          <asp:Button ID="btnApproveProcessing" runat="server" CssClass="action-btn" Text="通过-处理中" CommandName="ApproveProcessing" CommandArgument='<%# Eval("Incident_id") %>' />
  ...
  <script type="text/javascript">
    (function() {
      document.addEventListener('click', function(e) {
        var t = e.target;
        if (t && t.className && t.className.indexOf('toggle-desc') >= 0) {
          e.preventDefault();
          var box = t.previousElementSibling;
          if (!box) return;
          var clamped = box.className.indexOf('clamp') >= 0;
          if (clamped) { box.className = box.className.replace('clamp','').trim(); t.textContent = '收起'; }
          else { box.className += ' clamp'; t.textContent = '展开'; }
        }
      });
    })();
  </script>
